...
def _default_bootstrap_content(project_root: Path, pythonpath: list[str]) -> str:
    ...
    return "\n".join(
        [
            ...
            "echo \"[basalt bootstrap] mounted dirs (findmnt /home /home/bmll /home/bmll/user /home/bmll/organisation):\"",
            "findmnt /home /home/bmll /home/bmll/user /home/bmll/organisation 2>/dev/null || true",
            ...
            "echo \"[basalt bootstrap] /home/bmll/organisation listing:\"",
            "ls -la /home/bmll/organisation 2>/dev/null || true",
            f"PROJECT_ROOT=\"{project_root}\"",
            "MOUNT_WAIT_SECONDS=${BASALT_MOUNT_WAIT_SECONDS:-120}",
            "MOUNT_WAIT_STEP=${BASALT_MOUNT_WAIT_STEP_SECONDS:-2}",
            "MOUNT_TARGET=${BASALT_MOUNT_TARGET:-}",
            "if [ -z \"$MOUNT_TARGET\" ]; then",
            "  if [[ \"${PROJECT_ROOT:-}\" == /home/bmll/organisation* ]]; then",
            "    MOUNT_TARGET=/home/bmll/organisation",
            "  else",
            "    MOUNT_TARGET=/home/bmll/user",
            "  fi",
            "fi",
            "if [ \"$MOUNT_TARGET\" = \"/home/bmll/user\" ]; then",
            "  echo \"[basalt bootstrap] waiting for /home/bmll/user mount\"",
            "fi",
            "echo \"[basalt bootstrap] waiting for mount target ${MOUNT_TARGET} (timeout=${MOUNT_WAIT_SECONDS}s, step=${MOUNT_WAIT_STEP}s)\"",
            "elapsed=0",
            "until [ -d \"$MOUNT_TARGET\" ] && mount | grep -q \"$MOUNT_TARGET\"; do",
            "  if [ \"$elapsed\" -ge \"$MOUNT_WAIT_SECONDS\" ]; then",
            "    if [ \"$MOUNT_TARGET\" = \"/home/bmll/user\" ]; then",
            "      echo \"[basalt bootstrap] timeout waiting for /home/bmll/user mount\" >&2",
            "    fi",
            "    echo \"[basalt bootstrap] timeout waiting for mount target ${MOUNT_TARGET}\" >&2",
            "    findmnt /home /home/bmll /home/bmll/user /home/bmll/organisation 2>/dev/null || true",
            "    mount | grep -E '/home|bmll' || true",
            "    break",
            "  fi",
            "  sleep \"$MOUNT_WAIT_STEP\"",
            "  elapsed=$((elapsed + MOUNT_WAIT_STEP))",
            "done",
            "echo \"[basalt bootstrap] post-wait ${MOUNT_TARGET} listing:\"",
            "ls -la \"$MOUNT_TARGET\" 2>/dev/null || true",
...
def _bootstrap_needs_refresh(content: str) -> bool:
    ...
    if "BASALT_DIST=all python -m pip install -e ." not in content:
        return True
    if "waiting for mount target" not in content:
        return True
    return False
