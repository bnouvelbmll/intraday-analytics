...
class GenericAnalyticsConfig(BaseModel):
    ...
    aggregations: Dict[str, str] = Field(default_factory=dict) # col -> "sum", "mean", "last", "first"
...
class GenericAnalytics(BaseAnalytics):
    ...
    def compute(self) -> pl.LazyFrame:
        ...
        if source_df is None:
            ...
            raise ValueError(f"Source pass '{self.config.source_pass}' not found in context. Available: {list(self.context.keys())}")
        ...
        if self.config.resample_rule:
             ...
             if "TimeBucket" in self.config.group_by:
                 lf = lf.with_columns(pl.col("TimeBucket").dt.truncate(self.config.resample_rule))
        ...
        if self.config.aggregations:
            ...
            for col, method in self.config.aggregations.items():
                if method == "sum":
                    ...
                    logging.warning(f"Unknown aggregation method '{method}' for column '{col}'")
        ...
        if self.config.talib_indicators:
            if not TALIB_AVAILABLE:
                ...
                for ind in self.config.talib_indicators:
                    ...
                    def compute_talib(s: pl.Series, name=ind.name, period=ind.timeperiod) -> pl.Series:
                        try:
                    ...
                    if group_keys:
                        lf = lf.with_columns(
                            ...
                            .map_batches(lambda s: compute_talib(s), return_dtype=pl.Float64)
                            .over(group_keys)
                            .alias(out_col)
                        )
                    else:
