...
class L3Analytics(BaseAnalytics):
    ...
    def compute(self) -> pl.LazyFrame:
        ...
        df_bid = (
            ...
            .agg(
                ...
                (pl.when(pl.col("LobAction") == 2).then(pl.col("Size")).otherwise(pl.lit(0)))
                .sum()
                .alias("InsertVolumeBid"),
                (pl.when(pl.col("LobAction") == 3).then(pl.col("OldSize")).otherwise(pl.lit(0)))
                .sum()
                .alias("RemoveVolumeBid"),
                (pl.when(pl.col("LobAction") == 4).then(pl.col("Size")).otherwise(pl.lit(0)))
                .sum()
                .alias("UpdateInsertedVolumeBid"),
                (pl.when(pl.col("LobAction") == 4).then(pl.col("OldSize")).otherwise(pl.lit(0)))
                .sum()
        ...
        if self.config.enable_order_resting_time or self.config.enable_fleeting_liquidity:
    ...
    def _compute_order_resting_time_and_fleeting_liquidity(self, base_df: pl.LazyFrame) -> pl.LazyFrame:
        # We need to join inserts with removes/execs
        inserts = self.l3.filter(pl.col("LobAction") == 2).select(
            ["ListingId", "OrderID", "EventTimestamp", "Size"]
        ).rename({"EventTimestamp": "InsertTime", "Size": "InsertSize"})
        
        end_events = self.l3.filter(pl.col("LobAction").is_in([1, 3])).select(
            ["ListingId", "OrderID", "EventTimestamp", "TimeBucket"]
        ...
        lifetimes = lifetimes.with_columns(
            (pl.col("EndTime") - pl.col("InsertTime")).dt.total_nanoseconds().alias("DurationNs")
        )

        if self.config.enable_order_resting_time:
            resting_metrics = (
                lifetimes.group_by(["ListingId", "TimeBucket"])
                .agg(pl.col("DurationNs").mean().alias("AvgRestingTime"))
            )
            base_df = base_df.join(resting_metrics, on=["ListingId", "TimeBucket"], how="left")
            ...
            fleeting_metrics = (
                ...
                .agg(
                    (
                        ...
                        .otherwise(pl.lit(0))
                    ).sum().alias("FleetingVolume")
                )
            )
            
            # Join back to get Total Insert Volume for ratio
            base_df = base_df.join(fleeting_metrics, on=["ListingId", "TimeBucket"], how="left")
            base_df = base_df.with_columns(
                (
    ...
    def _compute_replacement_latency(self, base_df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        events = self.l3.filter((pl.col("LobAction") == 2) | (pl.col("ExecutionSize") > 0)).select(
            ["ListingId", "TimeBucket", "Side", "LobAction", "EventTimestamp", "ExecutionSize"]
        ).sort(["ListingId", "Side", "EventTimestamp"])
        
        # Calculate time to next insert after an execution
        events = events.with_columns(
            pl.col("EventTimestamp").shift(-1).over(["ListingId", "Side"]).alias("NextEventTime"),
            pl.col("LobAction").shift(-1).over(["ListingId", "Side"]).alias("NextLobAction"),
            pl.col("ExecutionSize").shift(-1).over(["ListingId", "Side"]).alias("NextExecutionSize"),
        )
        ...
        latency = (
            ...
            .with_columns(
                (pl.col("NextEventTime") - pl.col("EventTimestamp")).dt.total_nanoseconds().alias("LatencyNs")
            )
            .group_by(["ListingId", "TimeBucket"])
