...
def normalise_aggressor_side(trades: pl.LazyFrame) -> pl.LazyFrame:
    ...
    if "AggressorSide" not in schema.names():
        ...
        trades = trades.with_columns(
            AggressorSide=pl.when(pl.col("AggressorSide") == "BID").then(1)
                           .when(pl.col("AggressorSide") == "ASK").then(-1)
                           .otherwise(pl.col("AggressorSide").cast(pl.Int8, strict=False))
        )
    return trades

def group_executions(trades: pl.LazyFrame, deltat: float = 1e-5, ts_col: str = "EventTimestamp", group_cols: List[str] = None) -> pl.LazyFrame:
    if group_cols is None:
        group_cols = ["MIC", "ListingId"]

    direction_shift = (pl.col("AggressorSide") == pl.col("AggressorSide").shift(1).over(group_cols)).cast(pl.Int8)
    
    aggressor_groups = direction_shift.cum_sum().over(group_cols)
    same_side_seq = aggressor_groups - (aggressor_groups * (1 - direction_shift)).replace({0: None}).forward_fill().over(group_cols).fill_null(0)

    trades = trades.with_columns(SameSideSequence=same_side_seq)
    
    is_new_group = pl.col("SameSideSequence") == 0
    trades = trades.with_columns(
        TimeSincePrev=(pl.col(ts_col) - pl.col(ts_col).shift(1).over(group_cols)).dt.total_microseconds().fill_null(deltat * 1e6 + 1) / 1e6,
    )
    ...
    if end_of_event_col:
        is_same_group_expr = (
            (pl.col(end_of_event_col).shift(1).over(group_cols).cast(bool).fill_null(True)).not_()
...
def get_aggregation_expressions(trades_cols: List[str], execution_size: str, price_col: str) -> List[pl.Expr]:
    ...
    FIRST_COLS = [c for c in trades_cols if "PreTrade" in c or c.startswith("Old") or c in ["Ticker", "MarketState"]]
    LAST_COLS = [c for c in trades_cols if "PostTrade" in c or c in ["BestBidPrice", "BestAskPrice", "BestBidSize", "BestAskSize"]]
    UNIQUE_COLS = [
        "BMLLParticipantType", "ParticipantType", "MarketMechanism", "TradingMode",
        "TransactionCategory", "NegotiatedTrade", "CrossingTrade", "AlgorithmicTrade",
        "PublicationMode", "DeferralType", "BMLLTradeType", "Classification", "IsBlock", "LotType"
    ]

    # Basic aggregations
    aggs.extend([
        (pl.col(c).sum().alias(c)) for c in SUM_COLS if c in trades_cols and c != execution_size
    ])
    aggs.extend([
        (pl.col(c).first().alias(c)) for c in FIRST_COLS if c in trades_cols
    ])
    aggs.extend([
        (pl.col(c).last().alias(c)) for c in LAST_COLS if c in trades_cols
    ])
...
def aggregate_trades(
    ...
    base_stats = [
        ...
        (pl.col(ts_col).max() - pl.col(ts_col).min()).dt.total_microseconds().alias("Duration"),
    ...
    dynamic_stats = get_aggregation_expressions(schema.names(), execution_size, price_col)
    base_stats.extend(dynamic_stats)

    return trades.group_by(grouping_cols).agg(base_stats)

class AggressiveTradesTransform:
    def __init__(self, execution_size: Optional[str] = None, deltat: float = 1e-5, time_bucket_seconds: Optional[int] = None):
    ...
    def transform(self, data: pl.LazyFrame) -> Optional[pl.LazyFrame]:
        ...
        if "EventTimestamp" not in schema.names() and "TimestampNanoseconds" in schema.names():
        ...
        is_l3 = (
            ...
            and _first_available(schema, ["ExecutionSize", "execution_size"]) is not None
        )
        
        if is_l3:
            ts_col = "EventTimestamp" if "EventTimestamp" in schema.names() else "TimestampNanoseconds"
        ...
        trades = group_executions(trades, self.deltat, ts_col=ts_col, group_cols=group_cols)

        if is_l3 and "OldPrice" in trades.collect_schema().names() and "ExecutionPrice" in trades.collect_schema().names():
            trades = trades.filter(pl.col("OldPrice") == pl.col("ExecutionPrice"))

        aggressive_orders = aggregate_trades(trades, execution_size_col, ts_col=ts_col, group_cols=group_cols)
