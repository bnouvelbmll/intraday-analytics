...
class TestEagerExecution(unittest.TestCase):
    ...
    def _run_pipeline_with_config(self, eager_execution: bool):
        ...
        with patch('intraday_analytics.batching.pl.scan_parquet') as mock_scan_parquet, \
             ...
             patch('intraday_analytics.execution.get_files_for_date_range') as mock_get_files, \
             patch('intraday_analytics.batching.SymbolSizeEstimator') as mock_estimator_cls, \
             patch('bmll2.storage_paths') as mock_storage_paths:
            ...
            expected_out = os.path.join(self.temp_dir, f"final_{eager_execution}_2025-01-01_2025-01-02.parquet")
            self.assertTrue(os.path.exists(expected_out), f"Final output file not found for eager={eager_execution}")
