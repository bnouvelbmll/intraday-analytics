...
class TestPass2Analytics(unittest.TestCase):
    ...
    def test_pass2_aggregation_by_instrument(
        ...
        def side_effect_s3_path(sd, ed, config, pass_name):
            return os.path.join(
                self.temp_dir,
                f"final_{config.DATASETNAME}_{pass_name}_{sd.date()}_{ed.date()}.parquet",
            )
        ...
        self.config.PASSES[1].generic_analytics.group_by = [
            "InstrumentId",
            "TimeBucket",
        ]
    ...
    def test_pass2_talib(
        ...
        self.config.PASSES[1].generic_analytics.talib_indicators = [
            TalibIndicatorConfig(
                name="SMA", input_col="Close", timeperiod=2, output_col="SMA_2"
            )
        ]
