...
from intraday_analytics.analytics.trade import TradeAnalytics, TradeAnalyticsConfig, TradeGenericConfig
...
def test_trade_analytics_on_transformed_trades(trades_plus_fixture: pl.LazyFrame):
    ...
    transform = AggressiveTradesTransform(execution_size="Size", deltat=1e-3, time_bucket_seconds=60)
    ...
    if "Classification" in aggressive_orders_df.columns:
        expected_df = aggressive_orders_df.filter(pl.col("Classification") == "LIT_CONTINUOUS")
    else:
    ...
    assert total_computed_count == expected_trade_count, \
        "The total count from TradeAnalytics should match the number of aggressive orders."
    assert total_computed_volume == expected_volume, \
