...
def trades_plus_fixture() -> pl.LazyFrame:
    ...
    fixture_path = Path(__file__).parent.parent.parent / 'sample_fixture' / 'trades-plus.parquet'
...
def test_aggressive_trades_transform_with_trades_plus_fixture(trades_plus_fixture: pl.LazyFrame):
    """
    ...
    expected_cols = [
        "ConsolidatedTrades", "SequenceNumber", "TradeTimestamp", "Size", "Price",
        "AggressorSide", "Duration", "MinPrice", "MaxPrice", "NumExecutions", "PriceDiff"
    ]
    for col in expected_cols:
        assert col in result_df.columns, f"Expected column '{col}' not found in the output."
...
def test_aggressive_trades_transform_with_l3_fixture(l3_fixture: pl.LazyFrame):
    ...
    assert not result_df.is_empty(), "The resulting DataFrame from L3 data should not be empty."
    assert len(result_df) <= len(input_df), \
        "The number of aggressive orders should be less than or equal to the number of raw L3 executions."

    expected_cols = [
        "ConsolidatedTrades", "SequenceNumber", "EventTimestamp", "ExecutionSize", "Price",
        "AggressorSide", "Duration", "MinPrice", "MaxPrice", "NumExecutions", "PriceDiff"
    ]
    for col in expected_cols:
        assert col in result_df.columns, f"Expected column '{col}' not found in the L3 output."

    assert (result_df["ExecutionSize"] > 0).all(), "Aggregated 'ExecutionSize' should always be positive."
    assert (result_df["NumExecutions"] >= 1).all(), "Each aggressive order must consist of at least one execution."
