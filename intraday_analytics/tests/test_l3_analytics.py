...
class TestL3Analytics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_l3_data = pl.LazyFrame({
            ...
            "LobAction": [2, 1, 4, 3, 2, 3, 4, 1, 2, 3], # Insert, Exec, Update, Remove, Insert, Remove, Update, Exec, Insert, Remove
            ...
            "EventTimestamp": pl.Series([
                ...
                datetime(2026, 1, 30, 9, 0, 3, 0), # Changed from 9:00:09 to 9:00:03
            ...
            "SizeAhead": [10, 10, 20, 0, 15, 0, 25, 15, 12, 0] # For Queue Positioning
        })
    ...
    def test_arrival_flow_imbalance(self):
        ...
        self.assertAlmostEqual(result_df["ArrivalFlowImbalance"][0], expected_afi, places=6)
    ...
    def test_cancel_to_trade_ratio(self):
        ...
        self.assertAlmostEqual(result_df["CancelToTradeRatio"][0], expected_ctr, places=6)
    ...
    def test_queue_positioning(self):
        ...
        self.assertAlmostEqual(result_df["AvgQueuePosition"][0], expected_avg_queue_pos, places=6)
    ...
    def test_order_resting_time_and_fleeting_liquidity(self):
        ...
        self.assertAlmostEqual(result_df["AvgRestingTime"][0], expected_avg_resting_time, places=6)
        ...
        expected_fleeting_liquidity_ratio = expected_fleeting_volume / (expected_total_insert_volume + 1e-9)
        self.assertAlmostEqual(result_df["FleetingLiquidityRatio"][0], expected_fleeting_liquidity_ratio, places=6)
    ...
    def test_replacement_latency(self):
        ...
        self.assertAlmostEqual(result_df["AvgReplacementLatency"][0], expected_avg_replacement_latency, places=6)
